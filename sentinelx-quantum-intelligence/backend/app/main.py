...
def get_sim_status(db: Session = Depends(get_db)):
    ...
    # We want to persist the current step of simulation. We'll read the global SIM_STEP.
    # If the database has an active incident, ensure we are at step 6.
    inc_count = db.query(DBIncident).count()
    if inc_count > 0:
        SIM_STEP = 6
        
    return {"step": SIM_STEP, "max_steps": 6, "description": sim._get_step_description(SIM_STEP)}
