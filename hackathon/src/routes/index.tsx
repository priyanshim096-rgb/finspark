...
import {
  ...
  MessageSquare, FlaskConical, ListOrdered, Share2, BookOpen, Atom, Briefcase,
} from "lucide-react";
import { AlertMode, type AlertTone } from "@/components/AlertMode";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Kairos · Cyber & Transaction Correlation" },
      { name: "description", content: "Using cybersecurity and transaction data to detect fraud, score risks, respond quickly, and provide clear explanations for decisions." },
      { property: "og:title", content: "Kairos · Cyber & Transaction Correlation" },
      { property: "og:description", content: "Using cybersecurity and transaction data to detect fraud, score risks, respond quickly, and provide clear explanations for decisions." },
    ],
  }),
...
function Dashboard({ user }: { user: AuthUser }) {
  ...
  const [alertTone, setAlertTone] = useState<AlertTone>("critical");
  ...
  const toneForEvent = (e: SecurityEvent): AlertTone => {
    switch (e.type) {
      case "phishing": return "phishing";
      case "malware":  return "malware";
      case "password":
      case "login":    return "takeover";
      case "employee": return "insider";
  ...
  useEffect(() => {
    ...
    if (sev === "critical") {
      setAlertTone(toneForEvent(latest));
      setAlertEvt(latest);
      setSelected(latest);
    } else if (sev === "high") {
  ...
  const simulate = (kind: "phishing" | "malware" | "takeover") => {
    ...
    const tone: AlertTone = kind === "phishing" ? "phishing" : kind === "malware" ? "malware" : "takeover";
    setAlertTone(tone);
  ...
  const runCustomSim = (cfg: { name: string; target: string; type: SecurityEvent["type"]; count: number; risk: number }) => {
    const tone = toneForEvent({ type: cfg.type } as SecurityEvent);
    setAlertTone(tone);
    toast(`⚡ Custom sim: ${cfg.name}`, { description: `${cfg.count} synthetic ${cfg.type} events on ${cfg.target}` });
      ...
      setTimeout(() => {
        addEvents([
          makeEvent({
            type: cfg.type,
            user: cfg.target,
            title: `${cfg.name} · step ${i + 1}`,
            detail: `Custom-simulated ${cfg.type} event (${i + 1}/${cfg.count})`,
            risk: Math.min(100, cfg.risk + Math.round((Math.random() - 0.5) * 12)),
            reasons: [`Custom simulation "${cfg.name}"`, `Analyst-authored ${cfg.type} scenario`],
          }),
        ]);
      }, i * 700);
    }
  ...
  return (
    <div className="min-h-screen">
      ...
      <AlertMode active={criticalActive} event={alertEvt} tone={alertTone} onDismiss={() => setAlertEvt(null)} />
        ...
        <div className="mx-auto max-w-[1600px] px-6 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-roboto text-lg font-bold tracking-tight text-glow">Kairos</h1>
                <p className="font-roboto text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground">
                  Quantum Intelligence · Command Deck
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Link to="/profile" className="hidden md:flex items-center gap-2 rounded-full border border-border/60 px-2.5 py-1.5 hover:border-primary hover:bg-primary/10 transition-colors" title="Open profile">
              ...
              <button onClick={handleLogout} className="flex items-center gap-1.5 rounded-full border border-border px-2.5 py-1.5 text-xs font-medium text-muted-foreground hover:border-danger hover:text-danger">
                <LogOut className="h-3.5 w-3.5" /> Sign out
              </button>
            </div>
          </div>

          {/* Unified Tabs — dashboard views + route destinations, same style */}
            ...
            {tabs.map((t) => {
              ...
              return (
                <button key={t.id} onClick={() => setTab(t.id)}
                  className={`flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-medium whitespace-nowrap transition-colors ${active ? "bg-secondary text-secondary-foreground" : "text-muted-foreground border border-transparent hover:text-foreground hover:bg-secondary/15"}`}>
                  <Icon className="h-3.5 w-3.5" /> {t.label}
                </button>
              );
            })}
            <span className="mx-1 self-center h-4 w-px bg-border/70" />
            {[
              { to: "/graph",     label: "Graph",   Icon: Share2 },
              { to: "/stories",   label: "Stories", Icon: BookOpen },
              { to: "/quantum",   label: "Quantum", Icon: Atom },
              { to: "/executive", label: "Exec",    Icon: Briefcase },
            ].map(l => (
              <Link key={l.to} to={l.to}
                className="flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-medium whitespace-nowrap text-muted-foreground border border-transparent hover:text-foreground hover:bg-secondary/15 transition-colors">
      ...
      <main className="mx-auto max-w-[1600px] px-6 py-6 space-y-4">
        ...
        {tab === "copilot" && (
          <section className="grid gap-4 lg:grid-cols-3">
            <div className="lg:col-span-2">
              <Panel title="Copilot" icon={Radar} className="h-[70vh]">
        ...
        {tab === "simulate" && (
          <section className="grid gap-4 lg:grid-cols-2">
            <Panel title="Preset Attack Scenarios" icon={Zap} subtitle="One-click">
            ...
            <Panel title="Custom Attack Builder" icon={FlaskConical} subtitle="Design your own">
              <CustomSimForm onRun={runCustomSim} />
            </Panel>
          </section>
        )}
        ...
        <footer className="mt-8 flex flex-wrap items-center justify-between gap-2 border-t border-border/40 pt-4 text-[11px] font-mono uppercase tracking-widest text-muted-foreground">
          <span>Kairos Quantum Intelligence · v1.0</span>
          <span>Feeds: UPI · SWIFT · IdP · EDR · Firewall · DNS · Threat-Intel · PKI · HR</span>
        </footer>
      </main>
    </div>
  );
...
function CustomSimForm({ onRun }: { onRun: (cfg: { name: string; target: string; type: SecurityEvent["type"]; count: number; risk: number }) => void }) {
  const [name, setName] = useState("Rogue insider exfil");
  const [target, setTarget] = useState("kunal.shah");
  const [type, setType] = useState<SecurityEvent["type"]>("employee");
  const [count, setCount] = useState(4);
  const [risk, setRisk] = useState(78);
  const types: SecurityEvent["type"][] = ["phishing", "malware", "login", "password", "transaction", "device", "employee", "ip"];
  return (
    <form
      className="space-y-3"
      onSubmit={(e) => { e.preventDefault(); onRun({ name, target, type, count, risk }); }}
    >
      <div>
        <label className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Scenario name</label>
        <input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-input/60 px-3 py-1.5 text-sm outline-none focus:border-cyber" />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Target user</label>
          <input value={target} onChange={(e) => setTarget(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-input/60 px-3 py-1.5 text-sm outline-none focus:border-cyber" />
        </div>
        <div>
          <label className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Event type</label>
          <select value={type} onChange={(e) => setType(e.target.value as SecurityEvent["type"])} className="mt-1 w-full rounded-md border border-border bg-input/60 px-3 py-1.5 text-sm outline-none focus:border-cyber">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="flex justify-between text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
            <span>Event count</span><span className="text-foreground">{count}</span>
          </label>
          <input type="range" min={1} max={10} value={count} onChange={(e) => setCount(Number(e.target.value))} className="mt-1 w-full accent-cyan-400" />
        </div>
        <div>
          <label className="flex justify-between text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
            <span>Base risk</span><span className="text-foreground">{risk}</span>
          </label>
          <input type="range" min={20} max={100} value={risk} onChange={(e) => setRisk(Number(e.target.value))} className="mt-1 w-full accent-cyan-400" />
        </div>
      </div>
      <button type="submit" className="w-full rounded-md border border-cyber/60 bg-cyber/10 px-3 py-2 text-sm font-mono uppercase tracking-widest text-cyber hover:bg-cyber/20">
