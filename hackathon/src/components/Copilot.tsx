...
export function Copilot({ selected, all }: { selected: SecurityEvent | null; all: SecurityEvent[] }) {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    { role: "ai", text: "I'm your AI Security Copilot. Ask me why an event was flagged, or request a summary of today's activity." },
  ]);
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      toast.error(err);
      setMsgs((m) => [...m, { role: "ai", text: "⚠️ I couldn't reach the AI engine. Please try again." }]);
    } finally {
      setLoading(false);
    }
  };
